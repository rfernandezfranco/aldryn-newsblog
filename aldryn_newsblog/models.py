...
class Article(TranslatedAutoSlugifyMixin,
    ...
    def save(self, *args, **kwargs):
        # pop our custom keyword argument, if it exists
        pregenerate_thumbnail = kwargs.pop('pregenerate_thumbnail', False)
        ...
        if pregenerate_thumbnail and self.featured_image:
            try:
                from easy_thumbnails.files import get_thumbnailer
                thumbnailer = get_thumbnailer(self.featured_image)
                options = {
                    'size': (800, 450),
                    'crop': True,
                    'subject_location': self.featured_image.subject_location
                }
                thumbnailer.get_thumbnail(options)
            except Exception:
                # Silently fail, as per previous implementation.
                # The thumbnail will be generated on-the-fly on first request.
                pass
